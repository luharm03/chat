...
// Get data to work with
		...
		$uesrname2 = cleanInput($_GET['room']);
		$username = cleanInput($_GET['username']);
		$now = time();
// INSERT your data (if is not already there)
		 $findUser = "SELECT * FROM `chat_users_rooms` WHERE `username` = '$username' AND `uesrname2` ='$uesrname2' ";
		$getRoomUsr = runQry($findUser);
		$room = $getRoomUsr[0]->room;
